...
pub unsafe fn f16_to_i32_r_minMag(
    mut a: float16_t,
    mut exact: bool,
) -> int_fast32_t {
    ...
    let mut alignedSig: int_fast32_t = 0;
    uA.f = a;
    ...
    alignedSig = ((frac | 0x400 as i32 as u64) as int_fast32_t)
        << shiftDist as i32;
    if exact as i32 != 0
        && alignedSig & 0x3ff as i32 as i64 != 0
    {
        softfloat_exceptionFlags = (softfloat_exceptionFlags as i32
            | softfloat_flag_inexact as i32) as uint_fast8_t;
    }
    alignedSig >>= 10 as i32;
    return if sign as i32 != 0 { -alignedSig } else { alignedSig };
}
