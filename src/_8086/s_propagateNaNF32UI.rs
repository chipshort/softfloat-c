...
pub unsafe fn softfloat_propagateNaNF32UI(
    mut uiA: uint_fast32_t,
    mut uiB: uint_fast32_t,
) -> uint_fast32_t {
    let mut isSigNaNA: bool = false;
    let mut isSigNaNB: bool = false;
    let mut uiNonsigA: uint_fast32_t = 0;
    let mut uiNonsigB: uint_fast32_t = 0;
    let mut uiMagA: uint_fast32_t = 0;
    let mut uiMagB: uint_fast32_t = 0;
    isSigNaNA = uiA & 0x7fc00000 as i32 as u64
        == 0x7f800000 as i32 as u64
        && uiA & 0x3fffff as i32 as u64 != 0;
    isSigNaNB = uiB & 0x7fc00000 as i32 as u64
        == 0x7f800000 as i32 as u64
        && uiB & 0x3fffff as i32 as u64 != 0;
    uiNonsigA = uiA | 0x400000 as i32 as u64;
    uiNonsigB = uiB | 0x400000 as i32 as u64;
    if isSigNaNA as i32 | isSigNaNB as i32 != 0 {
        softfloat_raiseFlags(softfloat_flag_invalid as i32 as uint_fast8_t);
        if isSigNaNA {
            if !isSigNaNB {
                return if !uiB & 0x7f800000 as i32 as u64
                    == 0 as i32 as u64
            ...
            return if !uiA & 0x7f800000 as i32 as u64
                == 0 as i32 as u64
    ...
    uiMagA = uiA & 0x7fffffff as i32 as u64;
    uiMagB = uiB & 0x7fffffff as i32 as u64;
    if uiMagA < uiMagB {
        return uiNonsigB;
    }
    if uiMagB < uiMagA {
